...
fun MainLedgerView(
    ...
    // Audit Log and Timeline States
    var showAuditLogsSheet by remember { mutableStateOf(false) }
    var auditLogSearchQuery by remember { mutableStateOf("") }
    var auditLogSectorFilter by remember { mutableStateOf("ALL") } // "ALL", "دار", "حبايب", "مخزن"
    var auditLogActionFilter by remember { mutableStateOf("ALL") } // "ALL", "إضافة", "تعديل", "حذف"
    ...
    BackHandler(enabled = isSelectionMode || isDaySelectionMode || activeDayKey != null || showSearch || isHabayebActive || isMakhzanActive || showAuditLogsSheet) {
        if (showAuditLogsSheet) {
            showAuditLogsSheet = false
        } else if (isHabayebActive) {
                ...
                Row(
                    ...
                    IconButton(
                        ...
                        Icon(
                            ...
                            contentDescription = "سجل الأنشطة الموحد",
                            tint = Color.White,
                        ...
                        if (isDaySelectionMode) {
                            ...
                            Box(
                                ...
                                Row(
                                    ...
                                    // History Clock Icon (ساعة السجل) - Top-Right
                                    IconButton(
                                        onClick = {
                                            haptic.performHapticFeedback(HapticFeedbackType.LongPress)
                                            showAuditLogsSheet = true
    ...
    // Unified Immutable Audit Log & Timeline - Re-architected for maximum snugness and visual fakhama
    if (showAuditLogsSheet) {
        val auditLogs by viewModel.auditLogsState.collectAsState()
        var isSearchExpanded by remember { mutableStateOf(false) }

        Dialog(
            onDismissRequest = { showAuditLogsSheet = false },
            ...
            Box(
                ...
                Card(
                    ...
                    colors = CardDefaults.cardColors(containerColor = IvoryBackground),
                    ...
                    Column(
                        ...
                        Row(
                            ...
                            // RTL Top Bar Left: Close Button (X)
                            IconButton(
                                onClick = { showAuditLogsSheet = false },
                                modifier = Modifier
                                    .size(36.dp)
                                    .clip(CircleShape)
                            ...
                            // RTL Top Bar Right: Title & Expandable Search Button
                            Row(
                                modifier = Modifier.weight(1f),
                                horizontalArrangement = Arrangement.End,
                                verticalAlignment = Alignment.CenterVertically
                            ) {
                                // Search Input (Animated Expandable)
                                AnimatedVisibility(
                                ...
                                IconButton(
                                    onClick = {
                                        ...
                                        if (!isSearchExpanded) {
                                            auditLogSearchQuery = "" // Reset filter when closed
                                        }
                                    },
                                    modifier = Modifier
                                        .size(32.dp)
                                        .clip(CircleShape)
                                        .background(if (isSearchExpanded) EmeraldPrimary.copy(alpha = 0.15f) else Color.Gray.copy(alpha = 0.1f))
                                ) {
                                    Icon(
                                        imageVector = if (isSearchExpanded) Icons.Default.Close else Icons.Default.Search,
                                        contentDescription = "بحث",
                                        tint = if (isSearchExpanded) EmeraldPrimary else Color.DarkGray,
                                        modifier = Modifier.size(15.dp)
                                    )
                                }
                        ...
                        Row(
                            ...
                            Row(
                                ...
                                actionFilters.forEach { (key, label) ->
                                    val isSelected = auditLogActionFilter == key
                                    val chipBg = if (isSelected) {
                                        when (key) {
                                            "إضافة" -> Color(0xFFE8F5E9)
                                            "تعديل" -> Color(0xFFFFF3E0)
                                            "حذف" -> Color(0xFFFFEBEE)
                                    ...
                                    val chipBorderColor = if (isSelected) {
                                        when (key) {
                                            ...
                                            "تعديل" -> Color(0xFFE65100)
                                            "حذف" -> Color(0xFFC62828)
                                    ...
                                    Box(
                                        modifier = Modifier
                                            ...
                                            .clickable {
                                                auditLogActionFilter = if (isSelected) "ALL" else key
                                            }
                                ...
                                // Sector Filters
                                sectorFilters.forEach { (key, label) ->
                                    val isSelected = auditLogSectorFilter == key
                                    val chipBg = if (isSelected) {
                                        when (key) {
                                            "دار" -> Color(0xFFE8F5E9)
                                            "حبايب" -> Color(0xFFF3E5F5)
                                            "مخزن" -> Color(0xFFE0F7FA)
                                    ...
                                    val chipBorderColor = if (isSelected) {
                                        when (key) {
                                            "دار" -> Color(0xFF2E7D32)
                                            "حبايب" -> Color(0xFF7B1FA2)
                                            "مخزن" -> Color(0xFF00ACC1)
                                            else -> EmeraldPrimary
                                        }
                                    } else Color.LightGray.copy(alpha = 0.4f)
                        ...
                        val filteredLogs = remember(auditLogs, auditLogSearchQuery, auditLogSectorFilter, auditLogActionFilter) {
                            auditLogs.filter { log ->
                                if (log.sourceSystem == "النظام الأمني") return@filter false
                                val matchesSector = auditLogSectorFilter == "ALL" || log.sourceSystem == auditLogSectorFilter
                                val matchesAction = auditLogActionFilter == "ALL" || log.actionType == auditLogActionFilter
                                val matchesSearch = auditLogSearchQuery.isEmpty() ||
                                        log.description.contains(auditLogSearchQuery, ignoreCase = true) ||
                                        (log.oldValue?.contains(auditLogSearchQuery, ignoreCase = true) ?: false) ||
                                        (log.newValue?.contains(auditLogSearchQuery, ignoreCase = true) ?: false)
                                matchesSector && matchesAction && matchesSearch
                            }
                        }

                        val groupedLogs = remember(filteredLogs) {
                            filteredLogs.groupBy { getAuditLogGroupDate(it.timestamp) }
                        }
                        ...
                        if (filteredLogs.isEmpty()) {
                            Box(
                                ...
                                Column(
                                    ...
                                    Text(
                                        text = "السجل خالٍ تماماً من الحركات المطابقة",
                                        fontSize = 13.sp,
                                        fontWeight = FontWeight.Bold,
                                        color = Color.Gray
                                    )
                                    Text(
                                        text = "عدل خيارات التصفية أو البحث أعلاه لعرض السجل",
                                        fontSize = 10.sp,
                            ...
                            LazyColumn(
                                ...
                                groupedLogs.forEach { (dayGroup, logsInDay) ->
                                    // Minimalist Day Divider - centered text with clean dashes
                                    stickyHeader {
                                        Box(
                                            modifier = Modifier
                                                .fillMaxWidth()
                                    ...
                                    items(logsInDay) { log ->
                                        // Snug, Full-Width cards separated by warm divider
                                        Column(
                                            modifier = Modifier
                                                .fillMaxWidth()
                                                .background(Color.White)
                                            ...
                                            // Main row: Description and System Tag
                                            Row(
                                                modifier = Modifier.fillMaxWidth(),
                                                horizontalArrangement = Arrangement.End,
                                                verticalAlignment = Alignment.CenterVertically
                                                ...
                                                // Clean source system tags
                                                val (systemLabel, systemColor, systemBg) = when (log.sourceSystem) {
                                                    "دار" -> Triple("الرئيسية 🏠", Color(0xFF2E7D32), Color(0xFFE8F5E9))
                                                    "حبايب" -> Triple("العملاء 💜", Color(0xFF7B1FA2), Color(0xFFF3E5F5))
                                                    "مخزن" -> Triple("المخزن 📦", Color(0xFF00ACC1), Color(0xFFE0F7FA))
                                                    else -> Triple(log.sourceSystem, EmeraldPrimary, EmeraldPrimary.copy(alpha = 0.1f))
                                                }
                                                Text(
                                                    text = systemLabel,
                                                    fontSize = 9.sp,
                                            ...
                                            // Sub-row: Action type and Date/Time numeric
                                            Row(
                                                modifier = Modifier.fillMaxWidth(),
                                                horizontalArrangement = Arrangement.End,
                                                verticalAlignment = Alignment.CenterVertically
                                            ...
                                            // Extra snug Divider separated beautifully
                                            HorizontalDivider(
